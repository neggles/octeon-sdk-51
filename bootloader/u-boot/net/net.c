	...
	case PROT_IP:
		...
		if (ip->udp_xsum != 0) {
			...
			if ((xsum != 0x00000000) && (xsum != 0x0000ffff)) {
				...
				printf("Packet Length: %u, DST MAC: %pM, SRC MAC: %pM, protocol/length: 0x%04x\n"
				       "IPv%x, hdr len (words): %x\n"
				       "ToS: 0x%x\n"
				       "Total Length: %u\n"
				       "ID: %u, fragment offset: %u\n"
				       "ttl: %u, protocol: %u, IP checksum: 0x%x\n"
				       "source IP: %pI4, destination IP: %pI4\n"
				       "UDP source port: %u, destination port: %u\n"
				       "UDP length: %u, checksum: 0x%x\n",
				       len,
				       et->et_dest, et->et_src, et->et_protlen,
				       ip->ip_hl_v >> 4, ip->ip_hl_v & 0xF,
				       ip->ip_tos, ip->ip_len, ip->ip_id, ip->ip_off,
				       ip->ip_ttl, ip->ip_p, ip->ip_sum,
				       &ip->ip_src, &ip->ip_dst,
				       ip->udp_src, ip->udp_dst,
				       ip->udp_len, ip->udp_xsum);
				print_buffer(0, inpkt, 1, len, 0);
				return;
			}
		}
#endif
